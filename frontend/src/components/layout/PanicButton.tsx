      ...
      <div className="min-h-dvh bg-white text-gray-800">
        <div className="mx-auto max-w-2xl px-4 py-8">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-xl font-bold text-gray-700">{t("panic.todaysWeather")}</h1>
            <button
              onClick={togglePanic}
          ...
          <div className="space-y-4">
            <div className="bg-blue-50 rounded-lg p-6 text-center">
              <p className="text-5xl font-bold text-gray-800">{t("panic.temp")}</p>
              <p className="text-gray-500 mt-1">{t("panic.location")}</p>
              <p className="text-gray-400 text-sm mt-2">{t("panic.condition")}</p>
            </div>
            <div className="grid grid-cols-3 gap-3">
              {["forecastMon", "forecastTue", "forecastWed"].map((key) => (
                <div key={key} className="bg-gray-50 rounded-lg p-4 text-center">
                  <p className="text-sm font-semibold text-gray-600">{t("panic." + key)}</p>
                  <p className="text-lg font-bold text-gray-800">31°</p>
                  <p className="text-xs text-gray-400">{"26°"}</p>
                </div>
              ))}
            </div>
